...
if (empty($bb_attachments['max']['php_upload_limit'])) {	// internal php upload limit 
$bb_attachments['max']['php_upload_limit']=min(bb_attachments_l2n(ini_get('post_max_size')), bb_attachments_l2n(ini_get('upload_max_filesize'))); 
}
...
if ($output) {$output="<div class='bb_attachments'><h3>".__("Attachments")."</h3><ol>".$output."</ol></div>";}
...
if (!is_bb_feed()) {
	...
	$text.=bb_attachments_post_attachments($bb_post->post_id);
}
return $text;
}
