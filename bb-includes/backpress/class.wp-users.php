...
//	}
	...
	function get_user( $user_id = 0, $args = null ) {
		...
		if ( $from_cache ) {
			...
			// TODO LB3 Retrieving objects from teh Cache with WP enabled results in failed meta info lookup
			// probably because WP has already loaded teh object, but not the metadata
						$ID = false;

                if(array_key_exists("proto",$_REQUEST)) {
                        print "<h1>Safe User IDs</h1>";
                        var_dump($ID);
                }
